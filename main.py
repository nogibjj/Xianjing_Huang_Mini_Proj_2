...
def read_data(file):
    ...
    if file.endswith(".csv"):
        ...
        raise ValueError("Error in read dataset. \
        Please input a .csv/.xls/.xlsx file.")
...
def draw_histogram(df):
    ...
    plt.bar(df["Country"], df["Silver"], label='Silver', alpha=0.7, bottom=df["Gold"], color='silver')
    plt.bar(df["Country"], df["Bronze"], label='Bronze', alpha=0.7, bottom=[i+j for i,j in zip(df["Gold"], df["Silver"])], color='brown')
...
def generate_PDF(stored_stat):
    ...
    title = (
           "Report "
           "for Paris 2024 Olympics Medals by Country--top 10"
    )
    ...
    content_part1 = (
           ...
           "were an international multi-sport event that occurred from 26 July to 11 August 2024 in France, "
           "with the opening ceremony having taken place on 26 July. "
           "The dataset used selected the top 10 countries in the medal table and their gold, silver, "
...
#     pdf.cell(200, 10, txt=content_part1, ln=0, align="L")
    ...
    pdf.cell(200, 10, txt=link_txt, ln=True, align="L", link='https://olympics.com/en/paris-2024/medals')
    x = pdf.get_x()
    y = pdf.get_y()
